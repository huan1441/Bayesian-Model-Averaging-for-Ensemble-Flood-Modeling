...
#         Conditional PDF of observed data (D) is Gamma instead of Gaussian
#
# Created by Tao Huang, February, 2022
#
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
def logLikelihood(weight, sigma, D, f, K, T):
    ...
    for k in range(K):
        ...
        alpha = np.square(f[:,k]/sigma[k])
        beta = np.square(sigma[k])/f[:,k]
        
        P[:,k] = weight[k]*gamma.pdf(x=D,a=alpha,loc=0,scale=beta)
...
### function that generates K weights that follow conditional normal distribution and adding up to 1
def K_weight_normal(K,old_weight):
    
    weight = np.zeros(K)

    # draw candidate points from a normal distribution with sd=0.1
    weight = np.abs(norm.rvs(loc=old_weight,scale=0.1,size=K))
...
def MH_MCMC(D,f,old_weight,old_sigma,old_log_P):
    ...
    new_weight = K_weight_normal(K,old_weight)
